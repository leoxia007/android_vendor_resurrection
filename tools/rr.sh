...
echo -e "";
echo "{ \"response\": [ { \"datetime\": $(grep ro\.build\.date\.utc $OUT/system/build.prop | cut -d= -f2), \"filename\": \"$(basename $(ls $OUT/RR-P*.zip))\", \"id\": \"$((sha256sum $OUT/RR-P*.zip) | cut -d ' ' -f1)\", \"romtype\": \"$RR_BUILDTYPE\", \"size\": $(stat -c%s $OUT/RR-P*.zip), \"url\": \"https://downloads.sourceforge.net/project/xlagjn-rr/$(echo $RR_BUILD| tr '[a-z]' '[A-Z]')/$(basename $(ls $OUT/RR-P*.zip))\", \"version\": \"$(grep -m1 ro\.rr\.build\.version $OUT/system/build.prop | cut -d= -f2)\"  }]}" | tee $OUT/$RR_BUILD.json
